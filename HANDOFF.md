...
1. **X API가 402 Payment Required 반환** — `POST /2/tweets`가 거부됨. 7/23 게시 슬롯의 워크플로 실행 5개 전부 이걸로 실패, **7/23 게시 0건**. 파이프라인은 배포됐지만 **API 경유로 성공한 게시는 아직 한 번도 없음** (7/22의 성공 실행들은 슬롯 시간 전 no-op였음).
   → 원인 추정 유력: X 앱이 **Pay-Per-Use 방식인데 잔액 $0 + 결제수단 미등록**(7/22 세팅 당시 상태). 인증(GET /2/users/me)은 통과했지만 게시(POST)는 과금이라 거부되는 것. **해결: 사용자가 X Developer Portal에서 결제수단 등록(+지출 한도 설정 권장) 또는 무료 티어(월 500건 쓰기)로 전환.** 고치기 전까지 게시는 브라우저 수동으로 메꿔야 함.
