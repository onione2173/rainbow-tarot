...
**큰 변화가 생길 때마다 이 파일을 갱신할 것. 할 일 목록은 반드시 여기 기록하고, 기록된 루틴은 반드시 실행할 것 (사용자 지시 7/25).** (최종 업데이트: 2026-07-25 15:45 KST)
...
## 자동게시 시도 기록 (사용자 사용량 낭비 방지용 — 재테스트 금지)

세 경로 모두 사용자 제약(돈 0 + Claude 사용량 하드 제약)과 충돌해서 현재 채택 안 됨:

- **X API 파이프라인**: 무료 티어 없음, 선불 크레딧 $5 최소. 사용자 비용 거절(2026-07-24).
- **headless `claude -p` + 브라우저 (2026-07-26 Opus로 테스트)**: 작동하고 브라우저 도구도 있으나 **오르카 앱 내장 브라우저만** 제어 가능(실제 크롬 확장은 못 잡음). 그 오르카 브라우저는 **X 로그아웃 상태**. 로그인하려면 (a) 사용자가 오르카 브라우저에서 직접 로그인해야 하고(비번 입력은 어시스턴트 불가), (b) 신규·신뢰게이트 계정 새 브라우저 로그인은 X 보안잠금 위험, (c) **게시마다 Opus 사용량 소모** → 사용량 제약과 충돌. **결론: 로그인 벽 + 사용량 비용으로 현재 비채택.**
- **CronCreate(세션 크론)**: 세션 종료 시 소멸 → 무의미.
- **남은 무비용·무사용량 경로**: launchd + 순수 브라우저 스크립트(Playwright/AppleScript, Claude 안 씀). 미구축. X 봇탐지·셀렉터 유지보수 리스크 있음. 매출 생기면 재검토 후보.

## ✅ 매 세션 루틴 (사용자 지시 7/25: 여기 기록된 일은 반드시 실행할 것, 허락 재요청 불필요)

게시 세션마다 아래를 전부 돈다. 스킵하면 사유를 이 파일에 남긴다.

1. **게시**: 큐의 다음 `[ ]` 항목을 브라우저로 수동 게시 (하루 3건 목표, 아침/낮/저녁 분산). 게시 후 큐에 `[x]`+시각 기록.
   **게시 직후 Slack 알림 (사용자 지시 7/25)**: `gh workflow run fukumaru-post-notify.yml --repo onione2173/rainbow-tarot -f queue_number=<번호> -f post_url=<게시 URL> -f note="<선택>"` 실행. 기존 `SLACK_WEBHOOK_URL` 시크릿 재사용, 로컬에 웹훅 값 저장 안 함. 실전 테스트 완료(7/25).
   (`fukumaru-schedule.json`은 파이프라인 정지 중 — 무시.)
