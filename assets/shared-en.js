...
function showSpread(){
  ...
  countEl.textContent='Choose a card'; countEl.classList.remove('full');
...
function pickCard(i){
  ...
  const countEl=document.getElementById('spreadCount');
  countEl.textContent='✓ Selected'; countEl.classList.add('full');
  setTimeout(()=>{
      ...
      <div class="tarot-card" id="tc-0" onclick="flipCard(0)">
        ...
        <div class="card-face ${rv?'is-reversed':'is-upright'}">
          ...
          <div class="card-label">
            <div class="card-label-name">${card.name}</div>
            <div class="card-label-rv">${rv?'↕ Reversed':'Upright'}</div>
          </div>
        </div>
      </div>
    </div>`;
...
function flipCard(idx){
  ...
  track('card_flipped', {card_name: drawn[0]?.name||'', reversed: drawnRv[0]});
...
async function fetchReading(){
  ...
  document.getElementById('loadingText').innerHTML=
    `${PAGE_LOADING}<br><span style="font-size:.8rem;color:var(--text-dim)">Just a little longer</span>`;
  ...
  try{
    ...
    showReading(petName,'Something went wrong while sending your message.\n\nPlease try again in a little while.','');
  }
}

/* ── typing effect with **bold** support ── */
...
function showReading(petName,interpretation,letter){
  ...
  const rv=drawnRv[0];
  document.getElementById('readingTitle').textContent=`${drawn[0].name} ${rv?'(Reversed)':'(Upright)'}`;

  const chips=document.getElementById('chipRow');
  chips.innerHTML=`<span class="chip${rv?' rv':''}">${drawn[0].name} ${rv?'(Reversed)':'(Upright)'}</span>`;
...
function openLetter(){
  track('letter_opened');
  document.getElementById('letterTrigger').style.display='none';
  const petN=_currentPetName||'your pet';
  ...
  body.innerHTML='';
  if(!_pendingLetter){
    body.textContent='The letter could not be received.';
...
/* English version has no ads, so the interstitial is skipped */
...
function showInterstitial(){
  ...
  btn.disabled=true;
  btn.innerHTML='Closes in <span id="itlCd">5</span>s';
  let t=5;
...
/* Bottom sticky banner is unused in the English version (no element) */
    ...
    <div style="width:360px;padding:32px 26px 28px;
      ...
      <div style="position:relative;z-index:1;">
        ...
        <div style="text-align:center;margin-bottom:20px;padding:13px 16px;border-radius:14px;
          ...
          <div style="font-size:1.05rem;font-weight:600;color:#e8d0ff;">${card.name}</div>
          <div style="font-size:.7rem;color:#b090d0;margin-top:4px;">${rv?'Reversed':'Upright'}</div>
        </div>
...
async function shareResult(){
  ...
  if(typeof html2canvas==='undefined'){
    navigator.clipboard?.writeText(window.location.href)
      .then(()=>showToast('Link copied!')).catch(()=>{});
    return;
  }
  ...
  try{
    ...
    if(e.name!=='AbortError') showToast('Something went wrong while sharing');
  }
}
...
async function saveImage(){
  ...
  if(typeof html2canvas==='undefined'){ showToast('Please try again in a little while'); return; }
  showToast('Generating image…');
  try{
    const canvas=await renderShareCanvas();
    const card=drawn[0];
    const link=document.createElement('a');
    link.download=`tarot_${card.name.replace(/ /g,'_')}_${Date.now()}.png`;
