...
function showSpread(){
  ...
  countEl.textContent='카드를 골라주세요'; countEl.classList.remove('full');
...
function pickCard(i){
  ...
  const countEl=document.getElementById('spreadCount');
  countEl.textContent='✓ 선택됨'; countEl.classList.add('full');
  setTimeout(()=>{
      ...
      <div class="tarot-card" id="tc-0" onclick="flipCard(0)">
        ...
        <div class="card-face ${rv?'is-reversed':'is-upright'}">
          ...
          <div class="card-label">
            ...
            <div class="card-label-rv">${rv?'↕ 역방향':'정방향'}</div>
          </div>
        </div>
      </div>
    </div>`;
...
async function fetchReading(){
  ...
  document.getElementById('loadingText').innerHTML=
    `${PAGE_LOADING}<br><span style="font-size:.8rem;color:var(--text-dim)">잠시만 기다려주세요</span>`;
  ...
  try{
    ...
    showReading(petName,'메시지를 전달하는 중 오류가 발생했습니다.\n\n잠시 후 다시 시도해주세요.','');
  }
}
...
function showReading(petName,interpretation,letter){
  ...
  chips.innerHTML=`<span class="chip${rv?' rv':''}">${drawn[0].en} ${rv?'(역방향)':'(정방향)'}</span>`;
...
function openLetter(){
  track('letter_opened');
  document.getElementById('letterTrigger').style.display='none';
  const petN=_currentPetName||'반려동물';
  ...
  body.innerHTML='';
  if(!_pendingLetter){
    body.textContent='편지를 받아오지 못했습니다.';
...
function initReadingAd(){
  const wrap = document.getElementById('adInReading');
  const old = wrap.querySelector('ins.kakao_ad_area');
  if(old) old.remove();
  const ins = document.createElement('ins');
  ins.className = 'kakao_ad_area';
  ins.style.display = 'none';
  ins.setAttribute('data-ad-unit', 'DAN-QpeUnoNPzpR5Oflw');
  ins.setAttribute('data-ad-width', '320');
  ins.setAttribute('data-ad-height', '100');
  wrap.appendChild(ins);
  requestAnimationFrame(()=>{
    if(window.kakao_adfit && typeof kakao_adfit.display==='function'){
      kakao_adfit.display('DAN-QpeUnoNPzpR5Oflw');
    } else {
...
function onReset(){ track('reset_click'); showInterstitial(); }
...
function showInterstitial(){
  ...
  btn.disabled=true;
  btn.innerHTML='<span id="itlCd">5</span>초 후 닫기';
  let t=5;
...
function closeStickyAd(){
  track('sticky_ad_closed');
  document.getElementById('adSticky').classList.remove('show');
    ...
    <div style="width:360px;padding:32px 26px 28px;
      ...
      <div style="position:relative;z-index:1;">
        ...
        <div style="text-align:center;margin-bottom:20px;padding:13px 16px;border-radius:14px;
          ...
          <div style="font-size:.7rem;color:#b090d0;margin-top:4px;">${rv?'역방향':'정방향'}</div>
        </div>
...
async function shareResult(){
  ...
  if(typeof html2canvas==='undefined'){
    navigator.clipboard?.writeText(window.location.href)
      .then(()=>showToast('링크가 복사되었어요!')).catch(()=>{});
    return;
  }
  showToast('이미지 생성 중…');
  try{
    ...
    if(e.name!=='AbortError') showToast('공유 중 오류가 발생했어요');
  }
}
...
async function saveImage(){
  ...
  try{
    ...
    link.download=`타로_${card.en.replace(/ /g,'_')}_${Date.now()}.png`;
